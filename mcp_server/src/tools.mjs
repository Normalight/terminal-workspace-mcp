...
export function registerTools(server, { workspace: w, executor, jobs, terminals: t, config, diagnostics }) {
  ...
  define("get_file", { title: "Get original file", description: "Fetch an original accessible file (absolute, ~/ or default-directory relative path). Small images render directly. For large files provide offset=0 and continue using nextOffset; chunks include SHA256 and totalBytes.", inputSchema: { path: string, offset: z.number().int().min(0).optional(), maxBytes: z.number().int().min(4).max(1048576).optional() }, outputSchema: object, annotations: readOnly }, async a => {
    try {
      let file;
  ...
  server.registerTool("execute_command", {
    ...
    description: "Run shell commands on this machine in a persistent tmux terminal. Omit sessionId to open one, then reuse the returned ID to preserve cwd/env. To poll, pass sessionId and cursor without command. Use input to answer prompts or key=C-c to interrupt. waitMs/maxBytes bound this response, never stop the command. Output is original merged PTY stdout/stderr, including ANSI sequences. Use shell commands for files, search, Git and process management; command=exit closes the shell. Cwd is used when opening a session.",
