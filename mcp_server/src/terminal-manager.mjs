...
export class TerminalManager {
  ...
  async open({ cwd, name = "", env = {}, cols = 120, rows = 40, sessionId } = {}) {
    ...
    return this.lock.run("create", async () => {
      ...
      const meta = { sessionId: id, name, cwd, createdAt: new Date().toISOString(), log, logs: this.logs, cols, rows };
      ...
      const flags = [];
      for (const [key, value] of Object.entries({ ...this.env, ...env, HISTFILE: path.join(dir, "history") })) {
      ...
      try {
        await this.run(["new-session", "-d", "-s", id, "-c", cwd, "-x", String(cols), "-y", String(rows), ...flags, launch]);
  ...
  async execute(id, { command, waitMs = 1000, maxBytes = 65536 } = {}) {
    ...
    integer(waitMs, "waitMs", 0, 30000);
    const commandId = await this.lock.run(id, async () => {
      const state = await this.status(id); if (!state.alive) throw new OperationError("terminal is closed", "terminal_closed");
      ...
      const page = await this.read(id, { maxBytes: 4 });
